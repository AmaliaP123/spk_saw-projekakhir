...
function edit_item_kriteria(id){
    ...
    $.ajax({
        url : base_url + "subkriteria/" + "getSubById/" + id,
        type : "GET",
        dataType: "JSON",
        success : function(data){
            ...
            $('.modal-title').text('Ubah Sub Kriteria');
...
function save_item_kriteria() {
    var url;
    url =  base_url + "subkriteria/"+ "updateSubKriteria";
...
function lihat_kriteria(id){
    ...
    $.ajax({
        ...
        success:  function(data){
            ...
            $('#view_kriteria').modal('show');
            $('#view_kriteria .modal-title').text('Detail Sub Kriteria [ '+data.kriteria.kriteria+' ]');
        }
    });
}
